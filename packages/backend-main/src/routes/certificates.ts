...
certificatesRouter.post('/', async (c) => {
  const { domain_id, certificate_data, private_key, valid_from, valid_to, status = 'valid' } = await c.req.json()
  ...
  `).bind(domain_id, certificate_data, private_key, valid_from, valid_to, status).run()
